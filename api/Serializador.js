...
class Serializador {
    ...
    filtrarObjeto(dados) {
        ...
        this.camposPublicos.forEach(campo => {
            if(dados.hasOwnProperty(campo)) novoObj[campo = dados[campo]]
        })
    ...
    filtrar(dados) {
        if(Array.isArray(dados)) return dados.map(item => {
            return this.filtrarObjeto(item)
        })
        else return this.filtrarObjeto(dados)
    }
}
...
module.exports = {Serializador, SerializadorFornecedor, SerializadorProduto, SerializadorErro, formatosAceitos: ['application/json', 'application/xml']}